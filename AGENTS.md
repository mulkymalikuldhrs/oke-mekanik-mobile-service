...
- **Styling**: Tailwind CSS + Shadcn/ui (intensified Glassmorphism design system).
...
- **Real-time Ecosystem**: **Socket.io** for bidirectional communication (location tracking & chat).
- **Mapping**: Leaflet + React-Leaflet with custom markers for real-time tracking.
- **Backend**: Node.js + Express 5.2.1 server in `server/` directory.
- **Validation**: Strict **Zod** schema validation for all API inputs.
- **Authentication**: JWT-based auth with `bcryptjs` for password hashing and resource ownership checks.
- **Data Persistence**: SQLite database using `better-sqlite3` located at `server/okemekanik.db`.

## Key Features for Agents to Maintain
- **AI Diagnostics**: Maintain the weighted keyword scoring mapping in `server/index.js` and the holographic scanning animation in `BookingPage.tsx`.
