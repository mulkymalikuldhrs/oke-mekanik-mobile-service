...
3. **Verification**: Run `node verify_backend.js` to perform an automated full-stack API check.
4. **Database Schema**: Modify `server/db.js` to update the SQLite schema or seed data.
5. **Testing**: Run `npm test` and ensure all 22+ tests pass before any submission.
