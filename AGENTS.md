# 🤖 Oke Mekanik - AI Agent Guidelines (v5.6.0 ULTIMATE+)

This document provides strict instructions and conventions for AI agents working on the **Masterpiece v28** codebase.
...
- **UI Fidelity**: Maintain the **Masterpiece v28** Glassmorphism standard. Use `backdrop-blur-[160px]` and Framer Motion 12 for all transitions.
- **Localization**: All user-facing text must be integrated into the `translations` object in `src/hooks/useLanguage.tsx`.

## 🧠 AI Diagnostic Engine v5.6.0
...
- Ensure the `version` property always reflects the current standard (`v5.6.0-ultimate`).

## 🧪 Verification Protocol
Before submitting any work, you MUST run and pass:
1. `node tests/verify_ai_v56.js` - Validates the AI Engine logic.
...
---
*Oke Mekanik: Masterpiece v28 Ultimate — Built for the future of automotive service.*
