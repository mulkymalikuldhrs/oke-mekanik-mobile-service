...
const seedData = () => {
  ...
  if (userCount === 0) {
    ...
    insertMechanic.run('mech-3', null, 'Agus Prasetyo', 'Kelistrikan, AC', 4.7, 90000, 1, -6.2500, 106.8000, '👨‍🔧', 8, '08112233445');
    insertMechanic.run('mech-4', null, 'Siti Aminah', 'Tune Up, Rem', 4.6, 70000, 1, -6.1800, 106.8500, '👩‍🔧', 4, '08556677889');
    insertMechanic.run('mech-5', null, 'Dedi Kurniawan', 'Overhaul, Transmisi', 5.0, 120000, 1, -6.2000, 106.7500, '👨‍🔧', 15, '08778899001');
    ...
    insertService.run('svc-4', 'Servis AC', 'Pembersihan dan pengisian freon AC', 200000);
    insertService.run('svc-5', 'Tune Up', 'Pengoptimalan performa mesin', 125000);
    insertService.run('svc-6', 'Cek Kelistrikan', 'Diagnosa masalah sistem elektrikal', 75000);

    // Seed Bookings
    const insertBooking = db.prepare(`
      INSERT INTO bookings (id, user_id, mechanic_id, service_id, status, vehicle_brand, vehicle_model, problem, estimated_cost, location_lat, location_lng, location_address)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
    insertBooking.run('seed-book-1', 'cust-1', 'mech-1', 'svc-1', 'completed', 'Toyota', 'Avanza', 'Ganti Oli', 150000, -6.2200, 106.8300, 'Jl. Sudirman No. 1, Jakarta');
    insertBooking.run('seed-book-2', 'cust-1', 'mech-2', 'svc-2', 'completed', 'Honda', 'Civic', 'Servis Rutin', 250000, -6.1500, 106.9000, 'Jl. Thamrin No. 5, Jakarta');
    insertBooking.run('seed-book-3', 'cust-1', 'mech-3', 'svc-4', 'accepted', 'Mitsubishi', 'Xpander', 'AC Kurang Dingin', 300000, -6.2088, 106.8456, 'Rumah Customer (Pusat Jakarta)');
    insertBooking.run('seed-book-4', 'cust-1', 'mech-1', 'svc-3', 'pending', 'Suzuki', 'Ertiga', 'Ban Bocor', 100000, -6.2000, 106.8300, 'Kantor Sudirman');
