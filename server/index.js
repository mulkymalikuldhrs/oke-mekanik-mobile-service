...
const app = express();
const PORT = 3001;
const JWT_SECRET = 'your-super-secret-key-for-okemekanik';
...
app.post('/api/auth/login', (req, res) => {
  ...
  if (user && (bcrypt.compareSync(password, user.password_hash) || password === user.password)) {
...
app.post('/api/auth/register', (req, res) => {
  ...
  try {
    db.prepare('INSERT INTO users (id, name, email, password, password_hash, role, phone) VALUES (?, ?, ?, ?, ?, ?, ?)')
      .run(id, name, email, password, passwordHash, role, phone || null);
...
app.post('/api/mechanics/register', verifyToken, (req, res) => {
  ...
  // Get user name for mechanic name
