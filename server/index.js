...
app.get('/api/users/:id', verifyToken, (req, res) => {
  // Authorization: Only the user themselves can view their full profile
  // Note: Mechanics can be viewed by anyone for booking, but through /api/mechanics/:id
...
// --- AI Diagnostic Engine (v5.7 ULTIMATE - Advanced Technical Mapping) ---
const AI_MODEL = [
  {
    id: 'svc-7',
    name: 'Cek Aki',
...
app.post('/api/ai/diagnose', (req, res) => {
  ...
  let scores = AI_MODEL.map(service => {
    ...
    // Boost for technical terms (v5.8.1 ULTIMATE+)
    const technicalBoosts = {
      'svc-1': ['oli meler', 'oli rembes', 'oil seal', 'karter', 'oil pump', 'filter oli mampet'],
      'svc-4': ['brebet', 'berebet', 'ngelitik', 'pincang', 'ngeden', 'asap hitam', 'asap putih', 'nyendal', 'ngebul', 'ngobos', 'injector', 'bore up', 'overhaul', 'skir klep', 'turun mesin', 'packing head'],
      'svc-5': ['limp mode', 'check engine', 'konslet', 'korslet', 'ecu', 'wiring', 'sekring putus', 'short circuit', 'grounding', 'sensor crankshaft', 'immobilizer'],
      'svc-7': ['pagi susah nyala', 'stater berat', 'aki tekor', 'dinamo ampre', 'alternator bench', 'terminal aki berjamur'],
      'svc-2': ['gluduk', 'kaki-kaki', 'bunyi kaki-kaki', 'setir narik', 'v-belt', 'cv joint', 'bushing arm', 'tierod oblak', 'rack steer']
    };

    for (const [svcId, terms] of Object.entries(technicalBoosts)) {
      if (terms.some(term => p.includes(term)) && service.id === svcId) {
        score += 30; // Enhanced boost for v5.8.1
      }
    }
...
app.get('/api/bookings', verifyToken, (req, res) => {
  ...
  if (userId) {
    ...
    // Authorization: Mechanic can only see their own bookings
    ...
    // If no query param, default to current user's bookings based on role
