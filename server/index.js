...
// --- AI Diagnostic Engine (v5.1 Ultimate Weighted & Deep Technical Analysis) ---
const AI_MODEL = [
  { id: 'svc-7', name: 'Cek Aki', keywords: { aki: 10, battery: 10, stater: 8, tekor: 9, soak: 10, mati: 2, 'nggak nyala': 7, drop: 8, 'stater berat': 9, 'voltase': 7, 'alternator': 5, 'dinamo starter': 8, 'accu': 10, 'tahanan': 6, 'amper': 7, 'jumper': 8 } },
  { id: 'svc-1', name: 'Ganti Oli', keywords: { oli: 10, oil: 10, pelumas: 8, bocor: 4, hitam: 5, kental: 5, 'suara kasar': 6, 'mesin panas': 4, 'filter oli': 7, 'seal': 5, 'drain plug': 6, 'viskositas': 5, 'shell': 3, 'motul': 3, 'evalube': 3, 'pertamina': 3, 'yamalube': 3, 'castrol': 3, 'pelumasan': 7 } },
  { id: 'svc-3', name: 'Ganti Ban', keywords: { ban: 10, bocor: 9, tire: 10, kempes: 9, gundul: 8, pecah: 7, meletus: 9, 'kurang angin': 5, 'velg': 4, 'pentil': 5, 'tubeless': 6, 'tambal': 7, 'baling': 4, 'bridgestone': 3, 'michelin': 3, 'irc': 3, 'swallow': 3, 'corsa': 3, 'tekanan ban': 7 } },
  { id: 'svc-6', name: 'Ganti Kampas Rem', keywords: { rem: 10, brake: 10, cit: 8, mencit: 9, pakem: 8, bunyi: 4, decit: 10, blong: 10, 'rem bunyi': 9, 'piringan': 6, 'minyak rem': 7, 'kaliper': 5, 'disk brake': 8, 'tromol': 7, 'pad': 8, 'abs': 6, 'master rem': 7 } },
  { id: 'svc-8', name: 'Isi Freon AC', keywords: { ac: 10, freon: 10, panas: 7, dingin: 8, bau: 5, gerah: 4, 'ac mati': 9, 'nggak dingin': 10, 'kompresor': 6, 'evaporator': 6, 'filter ac': 5, 'blower': 5, 'magnetic clutch': 7, 'thermostat': 5, 'ekspansi': 6, 'kondensor': 7, 'kebocoran ac': 8, 'fan ac': 6 } },
  { id: 'svc-5', name: 'Cek Kelistrikan', keywords: { listrik: 10, lampu: 9, kabel: 8, konslet: 10, sekring: 9, mati: 3, sensor: 7, 'putus': 6, 'korslet': 10, 'ecu': 8, 'koil': 7, 'busi': 8, 'spul': 7, 'wiring': 9, 'harness': 8, 'relay': 7, 'alternator': 8, 'voltase': 7, 'grounding': 6, 'modul': 7, 'obd': 8 } },
  { id: 'svc-4', name: 'Tune Up', keywords: { mesin: 9, overheat: 10, asap: 8, brebet: 10, pincang: 10, mati: 5, ngadat: 10, bensin: 4, boros: 6, 'mogok': 10, 'tarikan berat': 9, 'injeksi': 9, 'karburator': 9, 'piston': 8, 'klep': 8, 'radiator': 9, 'kopling': 8, 'transmisi': 7, 'v-belt': 7, 'cvt': 7, 'throtle body': 9, 'carbon clean': 8, 'gurah mesin': 8, 'dct': 7, 'water pump': 9, 'head gasket': 9, 'pompa air': 8, 'ebm': 7, 'tps': 8, 'maf': 8, 'idle': 7 } },
  { id: 'svc-2', name: 'Servis Rutin', keywords: { rutin: 10, servis: 10, service: 10, berkala: 9, checkup: 8, 'ganti sparepart': 7, 'tune up rutin': 8, 'perawatan': 7, 'kilometer': 6, 'maintenance': 9, 'tahunan': 5, 'bulanan': 4, 'general checkup': 9, 'oli mesin': 8, 'filter udara': 7 } }
];
...
app.post('/api/ai/diagnose', (req, res) => {
  ...
  let scores = AI_MODEL.map(service => {
    ...
    // Enhanced Bonus for multiple keyword matches to increase confidence
    if (matches > 1) {
      score += (matches * 4);
    }
    // High confidence boost for exact matches of primary keywords (weight 10)
    const primaryMatches = Object.entries(service.keywords).filter(([k, w]) => w === 10 && p.includes(k));
  ...
  res.json({
    ...
    confidence: bestMatch.score > 0 ? Math.min(bestMatch.score / 45, 1) : 0,
    version: '5.1-masterpiece-ultimate'
  });
});
