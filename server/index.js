...
app.post('/api/ai/diagnose', (req, res) => {
  ...
  let scores = AI_MODEL.map(service => {
    ...
    // Boost for technical terms (v5.7.0 ULTIMATE+)
    const technicalBoosts = {
      'svc-1': ['oli meler', 'oli rembes'],
      'svc-4': ['brebet', 'berebet', 'ngelitik', 'pincang', 'ngeden', 'asap hitam', 'asap putih', 'nyendal', 'ngebul', 'ngobos'],
      'svc-5': ['limp mode', 'check engine', 'konslet', 'korslet', 'ecu', 'wiring', 'sekring putus'],
      'svc-7': ['pagi susah nyala', 'stater berat', 'aki tekor', 'dinamo ampre'],
      'svc-2': ['gluduk', 'kaki-kaki', 'bunyi kaki-kaki', 'setir narik']
    };

    for (const [svcId, terms] of Object.entries(technicalBoosts)) {
      if (terms.some(term => p.includes(term)) && service.id === svcId) {
        score += 25; // Increased boost for v5.7.0
      }
    }
  ...
  res.json({
    ...
    confidence: bestMatch.score > 0 ? Math.min(Math.round((bestMatch.score / 90) * 100), 99) : 40,
    possible_causes: bestMatch.causes,
    urgency_level: bestMatch.urgency,
    version: 'v5.7.0-ultimate'
  });
});
