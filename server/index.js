...
app.post('/api/bookings', verifyToken, (req, res) => {
  ...
  try {
    ...
    const err = new Error('Gagal membuat booking: ' + error.message);
    err.status = 400;
    next(err);
  }
});
...
app.post('/api/reviews', verifyToken, (req, res, next) => {
  ...
  try {
    ...
    const err = new Error('Gagal menambahkan ulasan');
    err.status = 400;
    next(err);
  }
});
...
app.use((err, req, res, next) => {
  ...
  console.error(`[${new Date().toISOString()}] SERVER ERROR (${req.method} ${req.url}):`, err);
