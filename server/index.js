...
const AI_MODEL = [
  ...
  },
  {
    ...
    keywords: { listrik: 12, lampu: 11, kabel: 10, konslet: 12, sekring: 11, mati: 4, sensor: 12, 'putus': 8, 'korslet': 12, 'ecu': 15, 'koil': 9, 'busi': 10, 'spul': 9, 'wiring': 11, 'harness': 10, 'relay': 9, 'alternator': 10, 'dinamo ampere': 10, 'short': 11, 'indikator nyala': 12, 'limp mode': 15, 'check engine': 12 },
...
app.post('/api/ai/diagnose', (req, res) => {
  ...
  let scores = AI_MODEL.map(service => {
    ...
    if (p.includes('brebet') || p.includes('berebet') || p.includes('ngelitik') || p.includes('pincang') || p.includes('limp mode') || p.includes('nyendal')) {
  ...
  res.json({
    ...
    confidence: bestMatch.score > 0 ? Math.min(Math.round((bestMatch.score / 75) * 100), 99) : 40,
    possible_causes: bestMatch.causes,
    urgency_level: bestMatch.urgency,
    version: 'v5.4.1-ultimate-plus'
  });
});
