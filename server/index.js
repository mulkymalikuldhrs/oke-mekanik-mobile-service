...
app.post('/api/ai/diagnose', (req, res) => {
  ...
  if (p.includes('aki') || p.includes('battery') || p.includes('stater') || p.includes('tekor')) {
    suggestion = "Cek Aki";
    serviceId = "svc-7";
  } else if (p.includes('oli') || p.includes('oil') || p.includes('pelumas') || p.includes('bocor oli')) {
    suggestion = "Ganti Oli";
    serviceId = "svc-1";
  } else if (p.includes('ban') || p.includes('bocor') || p.includes('tire') || p.includes('kempes')) {
    suggestion = "Ganti Ban";
    serviceId = "svc-3";
  } else if (p.includes('rem') || p.includes('brake') || p.includes('cit') || p.includes('mencit') || p.includes('pakem') || p.includes('bunyi')) {
    suggestion = "Ganti Kampas Rem";
    serviceId = "svc-6";
  } else if (p.includes('ac') || p.includes('freon') || p.includes('panas') || p.includes('tidak dingin')) {
    ...
    serviceId = "svc-5";
  } else if (p.includes('mesin') || p.includes('overheat') || p.includes('asap') || p.includes('brebet') || p.includes('pincang') || p.includes('mati') || p.includes('ngadat')) {
    suggestion = "Tune Up";
    serviceId = "svc-4";
...
app.patch('/api/bookings/:id/status', verifyToken, (req, res) => {
  ...
  // Security: Only customer of the booking or the assigned mechanic can update status
...
app.post('/api/messages', verifyToken, (req, res) => {
  ...
  // Security: Only parties of the booking can send messages
...
app.post('/api/payments', verifyToken, (req, res) => {
    ...
    // Security: Only customer of the booking can pay
...
app.post('/api/reviews', verifyToken, (req, res) => {
  ...
  // Security: Only customer who made the booking can review
  const booking = db.prepare('SELECT user_id FROM bookings WHERE id = ?').get(bookingId);
  if (!booking || booking.user_id !== req.userId) {
    return res.status(403).json({ message: 'Akses ditolak' });
  }
