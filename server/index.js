...
// --- AI Diagnostic Engine (v5.4 ULTIMATE - Advanced technical Mapping) ---
const AI_MODEL = [
  {
    id: 'svc-7',
    name: 'Cek Aki',
    keywords: { aki: 12, battery: 12, stater: 10, tekor: 11, soak: 12, mati: 2, 'nggak nyala': 8, drop: 10, 'stater berat': 11, 'voltase': 9, 'alternator': 7, 'dinamo starter': 10, 'accu': 12, 'terminal aki': 8, 'amper': 9, 'jumper': 10, 'starter panjang': 11, 'klakson lemah': 9, 'lampu redup': 8 },
  ...
  },
  {
    id: 'svc-1',
    name: 'Ganti Oli',
    keywords: { oli: 12, oil: 12, pelumas: 10, bocor: 6, hitam: 7, kental: 7, 'suara kasar': 8, 'mesin panas': 6, 'filter oli': 10, 'seal': 7, 'drain plug': 8, 'viskositas': 7, 'kuras': 8, 'pelumasan': 9, 'indikator oli': 11 },
  ...
  },
  {
    id: 'svc-3',
    name: 'Ganti Ban',
    keywords: { ban: 12, bocor: 11, tire: 12, kempes: 11, gundul: 10, pecah: 9, meletus: 11, 'kurang angin': 7, 'velg': 6, 'pentil': 7, 'tubeless': 8, 'tambal': 10, 'baling': 6, 'spooring': 5, 'balancing': 5, 'benjol': 12, 'makan dalam': 10, 'setir narik': 11, 'getar': 8, 'tekanan ban': 7 },
  ...
  },
  {
    ...
    keywords: { rem: 12, brake: 12, cit: 10, mencit: 11, pakem: 10, bunyi: 6, decit: 12, blong: 12, 'rem bunyi': 11, 'piringan': 8, 'minyak rem': 9, 'kaliper': 7, 'disk brake': 10, 'master rem': 9, 'pedal dalam': 11, 'getar': 8 },
  ...
  },
  {
    id: 'svc-8',
    name: 'Isi Freon AC',
    keywords: { ac: 12, freon: 12, panas: 9, dingin: 10, bau: 7, gerah: 6, 'ac mati': 11, 'nggak dingin': 12, 'kompresor': 8, 'evaporator': 8, 'filter ac': 7, 'kondensor': 9, 'magnetic clutch': 9, 'leak': 7, 'bocor halus': 8, 'ekstra fan': 9 },
  ...
  },
  {
    ...
    keywords: { listrik: 12, lampu: 11, kabel: 10, konslet: 12, sekring: 11, mati: 4, sensor: 9, 'putus': 8, 'korslet': 12, 'ecu': 11, 'koil': 9, 'busi': 10, 'spul': 9, 'wiring': 11, 'harness': 10, 'relay': 9, 'alternator': 10, 'dinamo ampere': 10, 'short': 11, 'indikator nyala': 10 },
  ...
  },
  {
    id: 'svc-4',
    name: 'Tune Up',
    keywords: { mesin: 10, overheat: 12, asap: 10, brebet: 15, berebet: 15, pincang: 15, mati: 6, ngadat: 12, bensin: 6, boros: 8, 'mogok': 12, 'tarikan berat': 11, 'injeksi': 11, 'karburator': 11, 'piston': 10, 'klep': 10, 'radiator': 11, 'kopling': 10, 'transmisi': 9, 'throttle body': 11, 'carbon clean': 10, 'water pump': 11, 'head gasket': 11, 'fan belt': 9, 'ngelitik': 15, 'detonasi': 11, 'asap putih': 13, 'asap hitam': 13, 'nyendal': 12, 'nyentak': 12, 'suara tek tek': 11 },
    causes: ['Throttle body kotor berkerak', 'Busi sudah lemah atau kotor', 'Saringan bahan bakar tersumbat', 'Sistem pendingin (radiator) tidak optimal'],
    urgency: 'HIGH'
  },
  {
    id: 'svc-2',
    name: 'Servis Rutin',
    keywords: { rutin: 12, servis: 12, service: 12, berkala: 11, checkup: 10, 'ganti sparepart': 9, 'maintenance': 11, 'tahunan': 7, 'bulanan': 6, 'perawatan': 9, 'km': 8, 'kilometer': 8, 'inspeksi': 10, 'gluduk': 14, 'kaki-kaki': 13, 'shockbreaker': 12, 'tierod': 11, 'balljoint': 11 },
...
app.post('/api/ai/diagnose', (req, res) => {
  ...
  let scores = AI_MODEL.map(service => {
    ...
    // Exponential Bonus for multiple keyword matches
    if (matches > 1) {
      score += Math.pow(matches, 2);
    }
    // Boost for technical terms
    if (p.includes('brebet') || p.includes('berebet') || p.includes('ngelitik') || p.includes('pincang')) {
        if (service.id === 'svc-4') score += 15;
    }
  ...
  res.json({
    ...
    confidence: bestMatch.score > 0 ? Math.min(Math.round((bestMatch.score / 85) * 100), 99) : 40,
    possible_causes: bestMatch.causes,
    urgency_level: bestMatch.urgency,
    version: '5.4-ultimate'
  });
});
