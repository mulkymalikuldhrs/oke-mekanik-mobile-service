...
// --- AI Diagnostic Engine (v5.2 ULTIMATE - Advanced technical Mapping) ---
const AI_MODEL = [
  {
    id: 'svc-7',
    name: 'Cek Aki',
    keywords: { aki: 12, battery: 12, stater: 10, tekor: 11, soak: 12, mati: 2, 'nggak nyala': 8, drop: 10, 'stater berat': 11, 'voltase': 9, 'alternator': 7, 'dinamo starter': 10, 'accu': 12, 'terminal aki': 8, 'amper': 9, 'jumper': 10 },
  ...
  },
  {
    id: 'svc-3',
    name: 'Ganti Ban',
    keywords: { ban: 12, bocor: 11, tire: 12, kempes: 11, gundul: 10, pecah: 9, meletus: 11, 'kurang angin': 7, 'velg': 6, 'pentil': 7, 'tubeless': 8, 'tambal': 10, 'baling': 6, 'spooring': 5, 'balancing': 5, 'benjol': 12, 'makan dalam': 10 },
  ...
  },
  {
    id: 'svc-4',
    name: 'Tune Up',
    keywords: { mesin: 10, overheat: 12, asap: 10, brebet: 12, pincang: 12, mati: 6, ngadat: 12, bensin: 6, boros: 8, 'mogok': 12, 'tarikan berat': 11, 'injeksi': 11, 'karburator': 11, 'piston': 10, 'klep': 10, 'radiator': 11, 'kopling': 10, 'transmisi': 9, 'throttle body': 11, 'carbon clean': 10, 'water pump': 11, 'head gasket': 11, 'fan belt': 9, 'ngelitik': 12, 'detonasi': 11 },
  ...
  },
  {
    id: 'svc-2',
    name: 'Servis Rutin',
    keywords: { rutin: 12, servis: 12, service: 12, berkala: 11, checkup: 10, 'ganti sparepart': 9, 'maintenance': 11, 'tahunan': 7, 'bulanan': 6, 'perawatan': 9, 'km': 8, 'kilometer': 8, 'inspeksi': 10 },
...
app.post('/api/ai/diagnose', (req, res) => {
  ...
  let scores = AI_MODEL.map(service => {
    ...
    if (p.includes('brebet') || p.includes('ngelitik') || p.includes('pincang')) {
  ...
  res.json({
    ...
    confidence: bestMatch.score > 0 ? Math.min(Math.round((bestMatch.score / 60) * 100), 99) : 40,
    possible_causes: bestMatch.causes,
    urgency_level: bestMatch.urgency,
    version: '5.2-ultimate'
  });
});
