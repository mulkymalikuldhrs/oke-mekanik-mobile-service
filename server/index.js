...
const EFFECTIVE_JWT_SECRET = JWT_SECRET || 'dev-secret-okemekanik-2024';
...
app.post('/api/auth/login', (req, res, next) => {
  try {
    ...
    if (user && bcrypt.compareSync(password, user.password_hash)) {
      const token = jwt.sign({ id: user.id, role: user.role }, EFFECTIVE_JWT_SECRET, { expiresIn: '24h' });
...
app.post('/api/auth/register', (req, res, next) => {
  try {
    ...
    const token = jwt.sign({ id, role }, EFFECTIVE_JWT_SECRET, { expiresIn: '24h' });
...
app.post('/api/auth/refresh', verifyToken, (req, res) => {
  const token = jwt.sign({ id: req.userId, role: req.userRole }, EFFECTIVE_JWT_SECRET, { expiresIn: '24h' });
  res.json({ token });
});
...
const AI_MODEL = [
  { id: 'svc-7', name: 'Cek Aki', keywords: { aki: 10, battery: 10, stater: 8, tekor: 9, soak: 10, mati: 2, 'nggak nyala': 7, drop: 8, 'stater berat': 9 } },
  { id: 'svc-1', name: 'Ganti Oli', keywords: { oli: 10, oil: 10, pelumas: 8, bocor: 4, hitam: 5, kental: 5, 'suara kasar': 6, 'mesin panas': 4 } },
  { id: 'svc-3', name: 'Ganti Ban', keywords: { ban: 10, bocor: 9, tire: 10, kempes: 9, gundul: 8, pecah: 7, meletus: 9, 'kurang angin': 5 } },
  { id: 'svc-6', name: 'Ganti Kampas Rem', keywords: { rem: 10, brake: 10, cit: 8, mencit: 9, pakem: 8, bunyi: 4, decit: 10, blong: 10, 'rem bunyi': 9 } },
  { id: 'svc-8', name: 'Isi Freon AC', keywords: { ac: 10, freon: 10, panas: 7, dingin: 8, bau: 5, gerah: 4, 'ac mati': 9, 'nggak dingin': 10 } },
  { id: 'svc-5', name: 'Cek Kelistrikan', keywords: { listrik: 10, lampu: 9, kabel: 8, konslet: 10, sekring: 9, mati: 3, sensor: 7, 'putus': 6, 'korslet': 10 } },
  { id: 'svc-4', name: 'Tune Up', keywords: { mesin: 9, overheat: 10, asap: 8, brebet: 10, pincang: 10, mati: 5, ngadat: 10, bensin: 4, boros: 6, 'mogok': 10, 'tarikan berat': 9 } },
  { id: 'svc-2', name: 'Servis Rutin', keywords: { rutin: 10, servis: 10, service: 10, berkala: 9, checkup: 8, 'ganti sparepart': 7, 'tune up rutin': 8 } }
];
