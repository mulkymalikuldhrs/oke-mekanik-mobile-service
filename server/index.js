...
app.post('/api/ai/diagnose', verifyToken, (req, res) => {
  ...
  if (!problem || problem.length < 5) {
    return res.status(400).json({ message: 'Problem description too short' });
  }
  ...
  if (p.includes('aki') || p.includes('battery') || p.includes('stater')) {
    suggestion = "Cek Aki";
    serviceId = "svc-7";
  } else if (p.includes('oli') || p.includes('oil') || p.includes('pelumas')) {
    suggestion = "Ganti Oli";
    serviceId = "svc-1";
  } else if (p.includes('ban') || p.includes('bocor') || p.includes('tire')) {
    suggestion = "Ganti Ban";
    serviceId = "svc-3";
  } else if (p.includes('rem') || p.includes('brake') || p.includes('cit')) {
    suggestion = "Ganti Kampas Rem";
    serviceId = "svc-6";
  } else if (p.includes('ac') || p.includes('freon') || p.includes('panas')) {
    suggestion = "Isi Freon AC";
    serviceId = "svc-8";
  } else if (p.includes('listrik') || p.includes('lampu') || p.includes('kabel')) {
    suggestion = "Cek Kelistrikan";
    serviceId = "svc-5";
  } else if (p.includes('mesin') || p.includes('pincang') || p.includes('asap')) {
    suggestion = "Tune Up";
    serviceId = "svc-4";
  }
...
app.get('/api/users/:id', verifyToken, (req, res) => {
  // Hardening: Prevent PII leak. Only return full info if it's the user themselves.
  // Otherwise, return limited info if there's a legitimate reason (e.g. mechanic/customer in a booking).
  ...
  if (user) {
    // If it's not the user themselves, strip sensitive info unless they are a mechanic
    if (user.id !== req.userId && user.role !== 'mechanic') {
        // Check if there is an active booking between them to allow phone access
        const hasBooking = db.prepare(`
            SELECT 1 FROM bookings
            WHERE (user_id = ? AND mechanic_id = (SELECT id FROM mechanics WHERE user_id = ?))
               OR (user_id = ? AND mechanic_id = (SELECT id FROM mechanics WHERE user_id = ?))
...
app.put('/api/users/:id', verifyToken, (req, res) => {
  // Hardening: Only allow user to update their own profile
...
app.patch('/api/mechanics/:id/status', verifyToken, (req, res) => {
  ...
  // Hardening: only mechanic can update their own status
  db.prepare('UPDATE mechanics SET is_online = ? WHERE (id = ? OR user_id = ?) AND user_id = ?')
    .run(isOnline ? 1 : 0, req.params.id, req.params.id, req.userId);
  res.json({ success: true });
});

app.patch('/api/mechanics/:id/location', verifyToken, (req, res) => {
  const { lat, lng } = req.body;
  // Hardening: only mechanic can update their own location
  db.prepare('UPDATE mechanics SET lat = ?, lng = ? WHERE (id = ? OR user_id = ?) AND user_id = ?')
    .run(lat, lng, req.params.id, req.params.id, req.userId);
  res.json({ success: true });
});
...
app.post('/api/bookings', verifyToken, (req, res) => {
  ...
  // Hardening: Always use req.userId from token
  db.prepare(`
...
app.get('/api/bookings', verifyToken, (req, res) => {
  ...
  // Hardening: Enforce resource ownership
  if (req.userRole === 'customer') {
    ...
    bookings = db.prepare('SELECT * FROM bookings WHERE mechanic_id = ? ORDER BY created_at DESC').all(mech ? mech.id : '');
  } else {
    // Admin or other could see all, but for now just empty if not customer/mechanic
    bookings = [];
  }
...
app.get('/api/bookings/active', verifyToken, (req, res) => {
    // Hardening: filter by active user
    let bookings;
    if (req.userRole === 'customer') {
      bookings = db.prepare("SELECT * FROM bookings WHERE user_id = ? AND status NOT IN ('completed', 'cancelled')").all(req.userId);
    } else {
      const mech = db.prepare('SELECT id FROM mechanics WHERE user_id = ?').get(req.userId);
      bookings = db.prepare("SELECT * FROM bookings WHERE mechanic_id = ? AND status NOT IN ('completed', 'cancelled')").all(mech ? mech.id : '');
    }
    res.json(bookings.map(formatBooking));
});
...
app.patch('/api/bookings/:id/status', verifyToken, (req, res) => {
  ...
  const bookingCheck = db.prepare('SELECT * FROM bookings WHERE id = ?').get(req.params.id);

  if (!bookingCheck) return res.status(404).json({ message: 'Not found' });

  // Hardening: Verify role and ownership
  const mech = db.prepare('SELECT user_id FROM mechanics WHERE id = ?').get(bookingCheck.mechanic_id);
  if (bookingCheck.user_id !== req.userId && (!mech || mech.user_id !== req.userId)) {
    return res.status(403).json({ message: 'Forbidden' });
  }
...
app.post('/api/messages', verifyToken, (req, res) => {
  ...
  // Hardening: ownership and sender verification
  const mech = db.prepare('SELECT user_id FROM mechanics WHERE id = ?').get(booking.mechanic_id);
  if (booking.user_id !== req.userId && (!mech || mech.user_id !== req.userId)) {
    return res.status(403).json({ message: 'Forbidden' });
  }
...
app.post('/api/payments', verifyToken, (req, res) => {
    ...
    // Hardening: Verify booking ownership
    const booking = db.prepare('SELECT * FROM bookings WHERE id = ?').get(bookingId);
...
app.get('/api/payments', verifyToken, (req, res) => {
    ...
    if (payment) {
        // Hardening: check booking ownership
...
app.post('/api/reviews', verifyToken, (req, res) => {
  ...
  try {
    // Hardening: verify booking ownership and completion
    const booking = db.prepare('SELECT * FROM bookings WHERE id = ?').get(bookingId);
    if (!booking || booking.user_id !== req.userId) {
