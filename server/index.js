...
const actualJwtSecret = finalJwtSecret || 'dev-secret-okemekanik-2024';
...
const verifyToken = (req, res, next) => {
  ...
  jwt.verify(token, actualJwtSecret, (err, decoded) => {
    if (err) {
...
app.post('/api/auth/login', (req, res, next) => {
  try {
    ...
    if (user && bcrypt.compareSync(password, user.password_hash)) {
      const token = jwt.sign({ id: user.id, role: user.role }, actualJwtSecret, { expiresIn: '24h' });
...
app.post('/api/auth/register', (req, res, next) => {
  try {
    ...
    const token = jwt.sign({ id, role }, actualJwtSecret, { expiresIn: '24h' });
...
app.post('/api/auth/refresh', verifyToken, (req, res) => {
  const token = jwt.sign({ id: req.userId, role: req.userRole }, actualJwtSecret, { expiresIn: '24h' });
  res.json({ token });
});
...
app.post('/api/mechanics/register', verifyToken, (req, res, next) => {
...
// --- AI Diagnostic Engine (v2.0 Weighted) ---
const AI_MODEL = [
  { id: 'svc-7', name: 'Cek Aki', keywords: { aki: 10, battery: 10, stater: 8, tekor: 9, soak: 9, mati: 2 } },
  { id: 'svc-1', name: 'Ganti Oli', keywords: { oli: 10, oil: 10, pelumas: 8, bocor: 4, hitam: 5, kental: 5 } },
  { id: 'svc-3', name: 'Ganti Ban', keywords: { ban: 10, bocor: 9, tire: 10, kempes: 9, gundul: 8, pecah: 7 } },
  { id: 'svc-6', name: 'Ganti Kampas Rem', keywords: { rem: 10, brake: 10, cit: 8, mencit: 9, pakem: 8, bunyi: 4, decit: 8 } },
  { id: 'svc-8', name: 'Isi Freon AC', keywords: { ac: 10, freon: 10, panas: 7, dingin: 8, bau: 5, gerah: 4 } },
  { id: 'svc-5', name: 'Cek Kelistrikan', keywords: { listrik: 10, lampu: 9, kabel: 8, konslet: 10, sekring: 9, mati: 3, sensor: 7 } },
  { id: 'svc-4', name: 'Tune Up', keywords: { mesin: 9, overheat: 10, asap: 8, brebet: 9, pincang: 9, mati: 5, ngadat: 8, bensin: 4, boros: 6 } },
  { id: 'svc-2', name: 'Servis Rutin', keywords: { rutin: 10, servis: 10, service: 10, berkala: 9, checkup: 8 } }
];
...
app.post('/api/reviews', verifyToken, (req, res, next) => {
