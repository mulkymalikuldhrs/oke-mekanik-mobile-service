...
app.post('/api/ai/diagnose', (req, res) => {
  ...
  let scores = AI_MODEL.map(service => {
    ...
    if (p.includes('brebet') || p.includes('berebet') || p.includes('ngelitik') || p.includes('pincang') || p.includes('ngeden')) {
        if (service.id === 'svc-4') score += 15;
    }
    if (p.includes('limp mode') || p.includes('check engine')) {
        if (service.id === 'svc-5') score += 15;
    }
    if (p.includes('pagi susah nyala') || p.includes('stater berat')) {
        if (service.id === 'svc-7') score += 15;
    }
...
app.get('/api/bookings', verifyToken, (req, res) => {
  ...
  if (userId) {
    ...
    // Admin or debugging access - ideally restricted to admin roles
