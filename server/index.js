...
app.post('/api/ai/diagnose', (req, res) => {
  ...
  let scores = AI_MODEL.map(service => {
    ...
    // Boost for technical terms (v5.8.0 ULTIMATE+)
    const technicalBoosts = {
      'svc-1': ['oli meler', 'oli rembes', 'oil seal', 'karter'],
      'svc-4': ['brebet', 'berebet', 'ngelitik', 'pincang', 'ngeden', 'asap hitam', 'asap putih', 'nyendal', 'ngebul', 'ngobos', 'injector', 'bore up', 'overhaul', 'skir klep'],
    ...
    for (const [svcId, terms] of Object.entries(technicalBoosts)) {
      if (terms.some(term => p.includes(term)) && service.id === svcId) {
        score += 30; // Enhanced boost for v5.8.0
      }
    }
