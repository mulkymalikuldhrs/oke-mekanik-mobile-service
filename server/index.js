...
app.patch('/api/mechanics/:id/status', verifyToken, (req, res) => {
  ...
  if (!mechanic || (mechanic.user_id !== req.userId && mechanic.id !== req.userId)) {
    return res.status(403).json({ message: 'Akses ditolak' });
  }

  db.prepare('UPDATE mechanics SET is_online = ? WHERE id = ?').run(isOnline ? 1 : 0, mechanic.id);
...
app.patch('/api/mechanics/:id/location', verifyToken, (req, res) => {
  ...
  db.prepare('UPDATE mechanics SET lat = ?, lng = ? WHERE id = ?').run(lat, lng, mechanic.id);
  res.json({ success: true });
});
...
app.post('/api/ai/diagnose', verifyToken, (req, res) => {
  ...
  if (!problem || problem.length < 5) {
    return res.status(400).json({ message: 'Deskripsi masalah terlalu pendek untuk dianalisa' });
  }
  ...
  if (p.includes('aki') || p.includes('battery') || p.includes('stater')) {
    ...
    serviceId = "svc-1";
  } else if (p.includes('ban') || p.includes('bocor') || p.includes('tire') || p.includes('roda')) {
    suggestion = "Ganti Ban";
    serviceId = "svc-3";
  } else if (p.includes('rem') || p.includes('brake') || p.includes('cit') || p.includes('mencit') || p.includes('bunyi')) {
    suggestion = "Ganti Kampas Rem";
    serviceId = "svc-6";
  } else if (p.includes('ac') || p.includes('freon') || p.includes('panas') || p.includes('dingin')) {
    suggestion = "Isi Freon AC";
    serviceId = "svc-8";
  } else if (p.includes('listrik') || p.includes('lampu') || p.includes('kabel') || p.includes('konslet')) {
    suggestion = "Cek Kelistrikan";
    serviceId = "svc-5";
  } else if (p.includes('transmisi') || p.includes('gigi') || p.includes('kopling') || p.includes('slip')) {
    suggestion = "Cek Transmisi";
    serviceId = "svc-4";
  } else if (p.includes('pincang') || p.includes('mati') || p.includes('asap') || p.includes('overheat') || p.includes('brebet')) {
    suggestion = "Tune Up Mesin";
    serviceId = "svc-9";
  }
...
app.patch('/api/bookings/:id/status', verifyToken, (req, res) => {
  ...
  // Basic authorization: Only the customer or the assigned mechanic can update status
  const mechanic = db.prepare('SELECT user_id FROM mechanics WHERE id = ?').get(booking.mechanic_id);
  if (booking.user_id !== req.userId && mechanic?.user_id !== req.userId) {
    return res.status(403).json({ message: 'Anda tidak memiliki akses untuk memperbarui booking ini' });
  }
