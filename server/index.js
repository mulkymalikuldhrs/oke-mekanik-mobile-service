...
// Fatal error if JWT_SECRET is missing in production
if (!JWT_SECRET && process.env.NODE_ENV === 'production') {
  console.error('FATAL: JWT_SECRET environment variable is required in production!');
  process.exit(1);
}

const FINAL_JWT_SECRET = JWT_SECRET || 'dev-secret-okemekanik-2024';
...
const verifyToken = (req, res, next) => {
  ...
  jwt.verify(token, FINAL_JWT_SECRET, (err, decoded) => {
    if (err) {
...
app.post('/api/auth/login', (req, res, next) => {
  try {
    ...
    if (user && bcrypt.compareSync(password, user.password_hash)) {
      const token = jwt.sign({ id: user.id, role: user.role }, FINAL_JWT_SECRET, { expiresIn: '24h' });
...
app.post('/api/auth/register', (req, res, next) => {
  try {
    ...
    const token = jwt.sign({ id, role }, FINAL_JWT_SECRET, { expiresIn: '24h' });
...
app.post('/api/auth/refresh', verifyToken, (req, res) => {
  const token = jwt.sign({ id: req.userId, role: req.userRole }, FINAL_JWT_SECRET, { expiresIn: '24h' });
  res.json({ token });
});
...
// --- AI Diagnostic Engine (v3.0 Advanced Weighted) ---
const AI_MODEL = [
  { id: 'svc-7', name: 'Cek Aki', keywords: { aki: 10, battery: 10, stater: 10, tekor: 10, soak: 10, mati: 3, 'low volt': 9, 'tidak bisa nyala': 5, 'klik-klik': 8 } },
  { id: 'svc-1', name: 'Ganti Oli', keywords: { oli: 10, oil: 10, pelumas: 8, bocor: 4, hitam: 6, kental: 6, 'filter oli': 9, 'jadwal ganti': 5, 'mesin kasar': 7 } },
  { id: 'svc-3', name: 'Ganti Ban', keywords: { ban: 10, bocor: 10, tire: 10, kempes: 10, gundul: 9, pecah: 8, meletus: 10, 'vlek': 5, 'paku': 9, 'tambal': 7 } },
  { id: 'svc-6', name: 'Ganti Kampas Rem', keywords: { rem: 10, brake: 10, cit: 9, mencit: 10, pakem: 10, bunyi: 5, decit: 10, 'blong': 10, 'seret': 8, 'pedal dalam': 9 } },
  { id: 'svc-8', name: 'Isi Freon AC', keywords: { ac: 10, freon: 10, panas: 8, dingin: 9, bau: 6, gerah: 5, 'tidak dingin': 10, 'angin saja': 9, 'kompresor': 7 } },
  { id: 'svc-5', name: 'Cek Kelistrikan', keywords: { listrik: 10, lampu: 9, kabel: 9, konslet: 10, sekring: 10, mati: 4, sensor: 8, 'short': 9, 'terbakar': 10, 'indikator': 7 } },
  { id: 'svc-4', name: 'Tune Up', keywords: { mesin: 9, overheat: 10, asap: 9, brebet: 10, pincang: 10, mati: 6, ngadat: 9, bensin: 5, boros: 7, 'mogok': 10, 'getar': 8, 'knocking': 9 } },
  { id: 'svc-2', name: 'Servis Rutin', keywords: { rutin: 10, servis: 10, service: 10, berkala: 9, checkup: 8, 'perawatan': 10, 'ganti semua': 5, 'tuneup': 7 } }
];
...
app.post('/api/ai/diagnose', (req, res) => {
  ...
  let scores = AI_MODEL.map(service => {
    ...
    for (const [keyword, weight] of Object.entries(service.keywords)) {
      if (p.includes(keyword)) { score += weight; }
    }
  ...
  res.json({
    ...
    confidence: bestMatch.score > 0 ? Math.min(bestMatch.score / 15, 1) : 0
  });
});
