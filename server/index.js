...
app.post('/api/ai/diagnose', (req, res) => {
  ...
  let scores = AI_MODEL.map(service => {
    ...
    // Boost for technical terms (v5.6.0 ULTIMATE+)
    const technicalBoosts = {
      'svc-4': ['brebet', 'berebet', 'ngelitik', 'pincang', 'ngeden', 'asap hitam', 'asap putih', 'nyendal', 'ngebul'],
      'svc-5': ['limp mode', 'check engine', 'konslet', 'korslet', 'ecu', 'wiring'],
    ...
    for (const [svcId, terms] of Object.entries(technicalBoosts)) {
      if (terms.some(term => p.includes(term)) && service.id === svcId) {
        score += 20; // Increased boost for v5.5.0
      }
    }
