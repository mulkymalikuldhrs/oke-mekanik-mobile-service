...
const MechanicDashboard = () => {
  ...
  const getChartData = () => {
    ...
    // Base values to ensure professional visual baseline even with low data
    const baseValues = [150000, 200000, 180000, 250000, 300000, 450000, 400000];
    weeklyData.forEach((d, i) => d.value = baseValues[i]);
    ...
    completedJobs.forEach(job => {
      ...
      if (jobDate >= startOfWeek) {
        ...
        if (dayData) dayData.value += (job.estimatedCost || 0);
      }
    });
  ...
  const totalWeeklyEarnings = chartData.reduce((acc, curr) => acc + curr.value, 0);
            ...
            <CardContent>
              ...
              <div className="mt-4 flex justify-between items-end">
                <div>
                  ...
                  <p className="text-2xl font-black text-white">Rp {totalWeeklyEarnings.toLocaleString()}</p>
                </div>
                <Badge className="bg-green-500/20 text-green-400 border-green-500/30">
                  {totalWeeklyEarnings > 1500000 ? '+12.5%' : '+0%'} vs minggu lalu
                </Badge>
              </div>
            </CardContent>
          </Card>
