...
const MechanicDashboard = () => {
  ...
  const { data: analytics } = useQuery({
    queryKey: ['mechanicAnalytics', mechanicData?.id],
    queryFn: () => mechanicApi.getAnalytics(mechanicData?.id || ''),
    enabled: !!mechanicData?.id,
  });
            ...
            <CardContent>
              ...
              <div className="mt-4 flex justify-between items-end">
                <div>
                  ...
                  <p className="text-2xl font-black text-white">Rp {analytics?.totalEarnings?.toLocaleString() || '0'}</p>
                </div>
                <Badge className="bg-green-500/20 text-green-400 border-green-500/30">Target: Rp 5.000.000</Badge>
              </div>
            </CardContent>
          </Card>
