...
const TrackingPage = () => {
  ...
  useEffect(() => {
    if (booking?.location && (booking.status === 'otw' || booking.status === 'working')) {
      // Initialize mechanic position slightly offset if not set
      if (!mechanicPos) {
        setMechanicPos([
          booking.location.lat + 0.005,
          booking.location.lng + 0.005
        ]);
      }

      // Simulate movement if OTW
      if (booking.status === 'otw') {
        const interval = setInterval(() => {
          setMechanicPos(prev => {
            if (!prev) return prev;
            const [lat, lng] = prev;
            const targetLat = booking.location.lat;
            const targetLng = booking.location.lng;

            // Move 10% closer to target each interval
            ...
            <div className="w-full h-96 relative z-10">
              {booking?.location?.lat && booking?.location?.lng ? (
                ...
                >
                  ...
                  {/* Mechanic Location (Real-time Simulation) */}
