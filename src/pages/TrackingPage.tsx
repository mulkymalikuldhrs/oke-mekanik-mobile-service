...
const TrackingPage = () => {
  ...
  useEffect(() => {
    ...
    socket.on('status_updated', (data: { status: string }) => {
      ...
      toast.info(`Status pesanan diperbarui: ${data.status.toUpperCase()}`);
    });
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white selection:bg-blue-500 overflow-x-hidden holographic-glow">
      {/* Background Glow */}
      ...
      <main className="container mx-auto px-4 py-8 space-y-8 relative z-10">
        {/* Real-time Map Card */}
        <Card className="glass-card shadow-2xl overflow-hidden group rounded-[2.5rem] relative">
                ...
                >
                  ...
                  <Marker position={[booking.location.lat, booking.location.lng]} icon={UserIcon}>
                    <Popup className="custom-popup">
                      <div className="text-xs font-bold uppercase">Lokasi Anda</div>
                    </Popup>
                  </Marker>
                  ...
                  {mechanicPos && ['otw', 'arrived', 'working'].includes(booking.status) ? (
                    ...
                    >
                      <Popup className="custom-popup">
                        <div className="text-xs font-bold uppercase text-orange-500">Mekanik Oke Mekanik</div>
                      </Popup>
                    </Marker>
                  ) : null}
                </MapContainer>
                ...
                <div className="bg-black/60 backdrop-blur-xl border border-white/10 p-4 rounded-2xl shadow-2xl pointer-events-auto">
                  <h3 className="text-sm font-black italic tracking-tighter uppercase text-white mb-1">
                    {booking.status === 'otw' ? 'MEKANIK SEDANG MENUJU ANDA' :
                     ...
                     booking.status === 'arrived' ? 'MEKANIK TELAH TIBA' :
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <div className="flex items-center space-x-1">
                       ...
                       <span className="text-[8px] font-black text-green-400 uppercase tracking-widest">Live GPS</span>
                    </div>
                  </div>
                </div>
              </div>
        ...
        {/* Status Progress */}
        <Card className="glass-card shadow-xl rounded-[2rem]">
          <CardHeader>
      ...
      <Dialog open={showArrivalModal} onOpenChange={setShowArrivalModal}>
        <DialogContent className="bg-[#0a0a0a]/90 backdrop-blur-3xl border-white/10 text-white p-0 overflow-hidden max-w-sm rounded-3xl">
           <div className="bg-gradient-to-b from-green-500/20 to-transparent p-8 text-center">
              ...
              <h2 className="text-2xl font-black italic tracking-tighter uppercase mb-2">Pekerjaan Selesai!</h2>
              <p className="text-gray-400 text-sm italic font-medium">Kendaraan Anda kini dalam kondisi prima.</p>
           </div>
           <div className="p-6 space-y-4">
              <div className="bg-white/5 border border-white/5 p-4 rounded-2xl">
                 <p className="text-[10px] text-gray-500 font-black uppercase tracking-widest mb-1">Total Biaya</p>
