...
const BookingPage = () => {
  ...
  const handleAiDiagnostic = async () => {
    ...
    try {
      const result = await aiApi.diagnose(vehicleData.problem);

      // Keep the cool animation visible for a bit to enhance "Masterpiece" feel
      await new Promise(resolve => setTimeout(resolve, 1500));

      setIsAnalyzing(false);
      setAiSuggestion(result.suggestion);
      setVehicleData(prev => ({ ...prev, problem: result.suggestion }));

      toast({
        title: 'Analisa AI Selesai',
        description: `Berdasarkan diagnosa, disarankan memilih layanan: ${result.suggestion}`,
      });
    } catch (error: any) {
      setIsAnalyzing(false);
      toast({
        title: 'Analisa AI Gagal',
        description: error.message || 'Gagal menghubungi asisten AI.',
        variant: 'destructive',
      });
    }
  };
