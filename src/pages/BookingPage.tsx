...
const BookingPage = () => {
  ...
  const { data: serviceTypes = [] } = useQuery({
    queryKey: ['services'],
    queryFn: () => import('@/lib/api').then(m => m.serviceApi.getAll()),
  });
  ...
  const bookingMutation = useMutation({
    mutationFn: async (data: { customerId: string; mechanicId: string; status: string; vehicle: VehicleData; problem: string; location: LocationData; estimatedCost: number; isEmergency: boolean }) => {
      // Find serviceId from serviceTypes based on problem
      const service = serviceTypes.find(s => s.name === data.problem) || serviceTypes[0];

      return bookingApi.create({
        mechanicId: data.mechanicId,
        serviceId: service?.id || 'svc-1',
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white selection:bg-blue-500 p-4 md:p-8 relative overflow-hidden">
      ...
      <div className="max-w-2xl mx-auto relative z-10">
        ...
        <Card className="shadow-2xl border border-white/10 bg-black/40 backdrop-blur-2xl rounded-3xl overflow-hidden">
