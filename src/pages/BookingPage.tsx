...
const BookingPage = () => {
  ...
  const [location, setLocation] = useState<LocationData & { lat: number; lng: number }>({ address: '', lat: -6.2088, lng: 106.8456 });
      ...
      navigator.geolocation.getCurrentPosition(
        (position) => {
          setLocation(prev => ({
            ...
            address: prev.address || 'Lokasi Terdeteksi (GPS)'
          }));
          toast({
  ...
  const bookingMutation = useMutation({
    mutationFn: async (data: { customerId: string; mechanicId: string; status: string; vehicle: VehicleData; problem: string; location: LocationData & { lat: number; lng: number }; estimatedCost: number; isEmergency: boolean; serviceId: string }) => {
