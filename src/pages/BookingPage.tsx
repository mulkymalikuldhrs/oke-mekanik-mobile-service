...
const BookingPage = () => {
    ...
    if ("geolocation" in navigator) {
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          if (!location.address) {
            setLocation({ address: `GPS Terdeteksi: ${position.coords.latitude.toFixed(4)}, ${position.coords.longitude.toFixed(4)}` });
          }
          toast({
            title: 'GPS Terkunci',
        ...
        () => {
          toast({
            title: 'GPS Gagal',
            description: 'Gagal mendeteksi lokasi. Pastikan izin lokasi diberikan.',
            variant: 'destructive',
          });
        }
      );
  ...
  const bookingMutation = useMutation({
    mutationFn: async (data: { customerId: string; mechanicId: string; status: string; vehicle: VehicleData; problem: string; location: LocationData; estimatedCost: number; isEmergency: boolean; serviceId: string; coords: { lat: number; lng: number } }) => {
