...
const BookingPage = () => {
  ...
  const bookingMutation = useMutation({
    mutationFn: async (data: { customerId: string; mechanicId: string; status: string; vehicle: VehicleData; problem: string; location: LocationData; coords: { lat: number; lng: number }; estimatedCost: number; isEmergency: boolean; serviceId: string }) => {
  ...
  const handleAiDiagnostic = () => {
    ...
    setTimeout(() => {
      ...
      if (problem.includes('aki') || problem.includes('battery') || problem.includes('stater')) {
        suggestion = "Cek Aki";
      } else if (problem.includes('oli') || problem.includes('oil') || problem.includes('pelumas')) {
        suggestion = "Ganti Oli";
      } else if (problem.includes('ban') || problem.includes('bocor') || problem.includes('tire')) {
        suggestion = "Ganti Ban";
      } else if (problem.includes('rem') || problem.includes('brake') || problem.includes('cit')) {
        suggestion = "Ganti Kampas Rem";
      } else if (problem.includes('ac') || problem.includes('freon') || problem.includes('panas')) {
        suggestion = "Isi Freon AC";
      } else if (problem.includes('listrik') || problem.includes('lampu') || problem.includes('kabel') || problem.includes('mati')) {
        suggestion = "Cek Kelistrikan";
      } else if (problem.includes('tune') || problem.includes('performa') || problem.includes('gas') || problem.includes('pincang')) {
        suggestion = "Tune Up";
      } else if (problem.includes('rutin') || problem.includes('berkala') || problem.includes('checkup') || problem.includes('mesin')) {
