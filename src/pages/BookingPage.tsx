...
const BookingPage = () => {
  ...
  const bookingMutation = useMutation({
    mutationFn: async (data: { customerId: string; mechanicId: string; status: string; vehicle: VehicleData; problem: string; location: LocationData; estimatedCost: number; isEmergency: boolean; serviceId: string }) => {
    ...
    <Card className="bg-white/5 backdrop-blur-xl border-white/10 text-white">
      ...
      <CardContent className="space-y-4">
        ...
        <div>
          ...
          <select
            id="problem"
            className="w-full p-2 border border-white/10 bg-black/40 text-white rounded-md mb-2"
            value={vehicleData.problem}
            onChange={(e) => setVehicleData(prev => ({ ...prev, problem: e.target.value }))}
          >
            <option value="" className="bg-gray-900">Pilih masalah</option>
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white selection:bg-blue-500 p-4 md:p-8 relative overflow-hidden font-sans">
      {/* Background Glow */}
      <div className="fixed top-0 left-0 w-full h-full pointer-events-none z-0">
        <motion.div
          animate={{ opacity: [0.05, 0.1, 0.05], scale: [1, 1.2, 1] }}
          transition={{ duration: 8, repeat: Infinity }}
          className="absolute top-[20%] right-[-10%] w-[40%] h-[40%] bg-blue-600/10 blur-[120px] rounded-full"
        />
        <motion.div
          animate={{ opacity: [0.05, 0.1, 0.05], scale: [1, 1.1, 1] }}
          transition={{ duration: 6, repeat: Infinity, delay: 3 }}
          className="absolute bottom-[20%] left-[-10%] w-[40%] h-[40%] bg-blue-600/10 blur-[120px] rounded-full"
        />
      </div>
      ...
      <div className="max-w-2xl mx-auto relative z-10 px-2 sm:px-0">
        ...
        >
          <Card className="shadow-2xl border border-white/10 bg-black/40 backdrop-blur-2xl rounded-2xl sm:rounded-[2.5rem] overflow-hidden">
