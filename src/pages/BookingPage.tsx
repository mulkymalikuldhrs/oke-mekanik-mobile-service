...
const BookingPage = () => {
  ...
  const handleAiDiagnostic = async () => {
    ...
    try {
      const response = await aiApi.diagnose(vehicleData.problem);

      setAiSuggestion(response.suggestion);
      setVehicleData(prev => ({ ...prev, problem: response.suggestion }));

      toast({
        title: 'Analisa AI Selesai',
        description: `Berdasarkan diagnosa, disarankan memilih layanan: ${response.suggestion}`,
      });
    } catch (err) {
      toast({
        title: 'Analisa Gagal',
  ...
  const renderLocationStep = () => (
    <Card className="bg-white/5 backdrop-blur-[40px] border-white/10 text-white">
      <CardHeader>
            ...
            {isAnalyzing && (
              ...
              >
                  <motion.div
                    ...
                    className="bg-blue-600/10 border border-blue-500/20 p-4 rounded-xl space-y-3 mb-3 relative overflow-hidden"
                  >
                    <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-blue-400 to-transparent animate-scan shadow-[0_0_10px_rgba(59,130,246,0.8)]" />
                  <div className="flex items-center space-x-3">
                    ...
                    <div className="flex-1">
                      <div className="h-1.5 w-full bg-blue-900/30 rounded-full overflow-hidden relative">
                        <motion.div
                      ...
                      <div className="flex justify-between mt-1">
                        <p className="text-[10px] text-blue-300 font-mono uppercase tracking-widest animate-pulse">Neural Diagnostic Active</p>
                        <p className="text-[10px] text-blue-400 font-mono">SCANNING...</p>
                      </div>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    <div className="bg-black/20 p-2 rounded border border-blue-500/10">
                      <p className="text-[8px] text-gray-500 font-mono uppercase">Engine Load</p>
                      <p className="text-[10px] text-blue-400 font-mono">0.82 G/REV</p>
                    </div>
                    <div className="bg-black/20 p-2 rounded border border-blue-500/10">
                      <p className="text-[8px] text-gray-500 font-mono uppercase">Fuel Trim</p>
                      <p className="text-[10px] text-blue-400 font-mono">+2.4% STFT</p>
                    </div>
                  </div>
                </motion.div>
              </motion.div>
  ...
  const renderMechanicSelection = () => (
    <Card className="bg-white/5 backdrop-blur-[40px] border-white/10">
      <CardHeader>
        <CardTitle className="flex items-center text-white">
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white selection:bg-blue-500 p-4 md:p-8 relative overflow-hidden">
      ...
      <motion.div
        ...
        initial="hidden"
        animate="visible"
        className="max-w-2xl mx-auto relative z-10 px-2 sm:px-0"
      >
        <Button
          variant="ghost"
        ...
        <Card className="shadow-2xl border border-white/10 bg-black/40 backdrop-blur-[40px] rounded-2xl sm:rounded-3xl overflow-hidden">
          <CardHeader className="bg-gradient-to-r from-blue-600 to-blue-800 text-white p-6 sm:p-8 relative overflow-hidden">
