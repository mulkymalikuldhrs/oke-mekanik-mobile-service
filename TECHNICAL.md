# 🛠️ Oke Mekanik - Technical Architecture (v5.7.0 ULTIMATE+)
...
- **`svc-1` (Oil Change)**: `oli meler`, `oli rembes`.
- **`svc-4` (Tune Up)**: `brebet`, `pincang`, `ngelitik`, `ngeden`, `asap putih/hitam`, `ngobos`.
- **`svc-5` (Electrical)**: `limp mode`, `check engine`, `korslet`, `ecu`, `sekring putus`.
