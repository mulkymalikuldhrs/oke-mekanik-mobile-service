...
- **AI Smart Diagnostic Engine (v5.1 Ultimate)**: Advanced weighted technical analysis to identify vehicle problems based on user symptoms with improved confidence scoring.
...
Oke Mekanik v28 utilizes a decoupled but highly integrated full-stack architecture:

1.  **Frontend (Vite + React 19)**: A high-performance SPA that communicates with the backend via a centralized API client.
2.  **Backend (Express 5.2)**: A secure REST API with integrated Socket.io for real-time bidirectional communication.
3.  **Database (SQLite)**: A persistent, file-based relational database managed via `better-sqlite3` for zero-latency data operations.
4.  **AI Engine**: A heuristic-based diagnostic engine integrated directly into the backend for immediate symptom analysis.

## Zero-Mock Policy

This project strictly adheres to a **Zero-Mock Policy**. All data interactions are real, persistent, and executed against the SQLite backend. No mock APIs, simulated delays, or placeholder data structures are permitted in the production-ready baseline.
