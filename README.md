...
Oke Mekanik is a comprehensive, futuristic mobile mechanic platform designed to bring professional vehicle services directly to the customer's location in real-time. This project represents the **Ultimate Unification** of autonomous development, featuring a robust full-stack architecture, zero-mock logic, and a high-intensity futuristic UI.

## 🚀 Key Features

- **Futuristic Masterpiece UI**: Intensified Glassmorphism design system with `backdrop-blur-[40px]`, multi-layered gradients, and high-intensity Framer Motion entrance animations across all core pages.
- **AI Smart Diagnostic (v3.0)**: Advanced real-time Indonesian technical analysis engine with holographic 'Scanning' overlay, providing repair suggestions based on weighted keyword scoring for technical terms like 'brebet', 'mogok', and 'ngadat'.
...
- **Unified Testing Suite**: Consolidated unit tests in `src/__tests__/` (100% pass rate) and automated E2E verification using Playwright.
